...
class EditDescActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val toolbar: androidx.appcompat.widget.Toolbar = findViewById(R.id.change_desc_toolbar)
        textInput.setText(intent.extras.getString(Constants.DESCRIPTION))
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        return if(item?.itemId == R.id.change_menu_submit){
            ...
            if(isFieldCorrect(text, field)){
                ...
                val updateData = hashMapOf<String,Any>(Constants.DESCRIPTION to text)
                ...
                firestore.collection(Constants.POSTS_PATH).document(postID).update(updateData).addOnSuccessListener {
                    firestore.collection(Constants.USERS_PATH).document(user).collection(Constants.POSTS_PATH).document(userPostID).update(updateData).addOnCompleteListener { task ->
                        ...
                        if(task.isSuccessful){
                            ...
                            updatedActivity.putExtra(Constants.DESCRIPTION, text)
                            finish()
                        }
                    }
                }
    ...
    private fun isFieldCorrect(text: String, field: TextInputLayout): Boolean {
        when {
            ...
            text.length > 200 -> {
                field.error = R.string.description_field_error_message.toString()
                return false
            }
